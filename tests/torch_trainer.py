    ...
    for mode in ["dist"]:  # TODO: re-enable local mode when https://github.com/pytorch/torchdistx/issues/21 is fixed.
        ...
        with atorch.bind_ops():
            ...
            def sgd_train_func(module_func, loss_func, optim_func, params, bufs, optim_state, batch):
                ...
                # TODO: we need to get value_and_grad to work to avoid running forward twice.
                # loss_value, bufs = compute_loss(params, bufs, inputs, targets)
                # params_grad, _ = alpa.grad(compute_loss, has_aux=True)(params, bufs.copy(), inputs, targets)
            ...
            def iter_func(params, bufs, optim_state, pt_batch):
                if alpa.torch.mode() == "local":  # local development
                    ...
                    params, bufs, optim_state, loss_value = alpa.parallelize(
                        train_func,
                        # method=alpa.PipeshardParallel(stage_mode="auto", num_micro_batches=num_micro_batches, submesh_physical_shape_space="all", submesh_logical_shape_space="all"),
                        # method=alpa.PipeshardParallel(stage_mode="uniform", num_micro_batches=num_micro_batches),
                        # method=alpa.ShardParallel(num_micro_batches=num_micro_batches),
