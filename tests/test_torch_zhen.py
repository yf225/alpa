...
def construct_w_b_pair(
    shape: List[int], uniform_const: float
) -> Tuple[nn.Parameter, nn.Parameter]:
    assert len(shape) == 2
    w = nn.Parameter(torch.empty(shape).uniform_(-1 * uniform_const, uniform_const))
    b = nn.Parameter(
        torch.empty([shape[0]]).uniform_(-1 * uniform_const, uniform_const)
    )  # UniformFillß
...
class ZHENLayer(nn.Module):
    def __init__(
        ...
        layer_index: int,
        emb_dim: int,
        token_mixer_list: List[int],  # determines this layer's output features
        previous_n_embs: int = 369,  # previous layer's output dim, may not be inferrable if token_mixer is different per layer. If 0th layer, this is original_n_embs.
        previous_input_embs: int = 369,  # skip connection's num embs. This is previous layer's input num embs.
        output_embs_per_mixer: int = 50,  # each module outputs 50 embeddings
        original_n_embs: int = 369,  # whatever overarch gives us for the 0th zhen layer . the rest, is whatever output previous layer is
    ):
        super().__init__()
        ...
        if self.layer_index != 0:
            self.match_w, self.match_b = construct_w_b_pair(
                [previous_n_embs, previous_input_embs], 0.2986738458275795
            )

        if self.layer_index != 0:
            self.layer_norm_w = nn.Parameter(
                torch.empty([emb_dim]).fill_(0.17280860245227814)
            )  # ConstantFill
            self.layer_norm_b = nn.Parameter(
                torch.empty([emb_dim]).fill_(0.0)
            )  # ConstantFill
        for token_mixer in self.token_mixer_list:
            if token_mixer == 1: # == TokenMixer.DOT:
                self.ffn_w, self.ffn_b = construct_w_b_pair(
                    [
                        512,
                        original_n_embs**2
                        if self.layer_index == 0
                        else previous_n_embs**2,
                    ],
                    0.03125,
                )
            ...
            elif token_mixer == 2: # == TokenMixer.LINEAR:  # n = 50
                self.w_linear, self.b_linear = construct_w_b_pair(
                    [output_embs_per_mixer, previous_n_embs], 0.02986738458275795
                )

            elif token_mixer == 3: # == TokenMixer.ATTENTION:  # n = 50
                self.encoder_layer = TransformerEncoderLayer(
                    d_model=emb_dim, nhead=1, batch_first=True
                )
    ...
    def forward(
        ...
        skip_connection: Optional[
            torch.Tensor
        ],  # the skip connection, i.e., previous layer's input
        ...
        if skip_connection is not None:
            # TODO: record decision of this control flow in module init time,
            # and check decision here.
            # if skip_connection.shape[-1] != input.shape[-1]:
            skip_connection = torch.nn.functional.linear(
                skip_connection, self.match_w, bias=self.match_b
            )

                # print(f"skip = {skip_connection.shape}, input = {input.shape}")
        ...
        for token_mixer in self.token_mixer_list:
            if token_mixer == 1: # == TokenMixer.DOT:  # num_dot_emb = 50
                input_feature_t = input_feature.permute(0, 2, 1)
                dot_products = torch.bmm(input_feature_t, input_feature)
                flattened_dot_products = torch.flatten(
                    dot_products, start_dim=-2
                )  # Flatten
                r = torch.addmm(
                    self.ffn_b, flattened_dot_products, self.ffn_w.t()
                )  # FC
            ...
            elif token_mixer == 2: # == TokenMixer.LINEAR:
                linear_emb_list = torch.nn.functional.linear(
                    input_feature, self.w_linear, bias=self.b_linear
                )
            ...
            elif token_mixer == 3: # == TokenMixer.ATTENTION:
                ...
                flat_compress_list = attention_emb_list.permute(0, 2, 1).reshape(
                    B, -1
                )  # (B, feature, dim)
                output.append(flat_compress_list)

            elif token_mixer == 4: # == TokenMixer.CONVOLUTION:
                reshape_input_feature = input_feature.reshape(B, 1, self.emb_dim, -1)
                r_conv = self.conv(reshape_input_feature)
                reshape_r_conv = r_conv.reshape(B, self.emb_dim, -1)
                compress_list = torch.nn.functional.linear(
                    reshape_r_conv, self.w_conv, bias=self.b_conv
                )
...
class ZHENCollection(nn.Module):
    def __init__(
        ...
        num_layers: int,
        emb_dim: int,
        token_mixer_list: Union[List[int], List[List[int]]],
        original_emb_num: int,
        output_emb_per_ensemble_module: int,
        enable_first_layer_skip: int,
    ):
        super().__init__()
        self.enable_first_layer_skip = enable_first_layer_skip
        ...
        for i in range(num_layers):
            layer = ZHENLayer(
                ...
                previous_n_embs=(
                    ...
                    if i == 0
                    # pyre-ignore[6]
                    else len(token_mixer_list[i-1]) * output_emb_per_ensemble_module
                ),
                previous_input_embs=(
                    original_emb_num 
                    if i <= 1
                    # pyre-ignore[6]
                    else len(token_mixer_list[i-2]) * output_emb_per_ensemble_module
                ),
                output_embs_per_mixer=output_emb_per_ensemble_module,
                original_n_embs=original_emb_num,  # if i == 0 else len(token_mixer_list) * output_emb_per_ensemble_module
            )
            # self.dense_num_params += sum(x.numel() for x in layer.get_dense_params())
...
# homogenoues ZHEN
B=64  # 59 # made divisible by 8
F=37
D=64
LAYERS=1  # 5
OUTPUT_PER_ENSEMBLE=50
TOKENS=[int(TokenMixer.ATTENTION)]  # , int(TokenMixer.LINEAR), int(TokenMixer.ATTENTION), int(TokenMixer.CONVOLUTION), int(TokenMixer.DOT)]


# `meta_init` allows a PyTorch model to be created with shape-only tensors as weights.
pt_module = atorch.meta_init(ZHENCollection, LAYERS, D, TOKENS, F, OUTPUT_PER_ENSEMBLE, False)
...
dataloader = [
    (torch.empty(B, D, F), torch.empty(B, 3200)), # 16000)),
    (torch.empty(B, D, F), torch.empty(B, 3200)), # 16000)),
]
