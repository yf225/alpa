...
def fx_ir_to_alpa_func_code(fx_ir, alpa_func_name):
    ...
    for line in lines[1:]:
        ...
        if "self." in line:
            if "getattr(" in line:
                # Example line in IR:
                # `layers_0_encoder_layer_self_attn_qkv_weight : torch.Tensor = getattr(self.layers, "0").encoder_layer.self_attn.qkv.weight`
                # For RHS, FQN in param dict should be: "layers.0.encoder_layer.self_attn.qkv.weight"
                attr_fqn_name_in_original_ir = line.split(" = ")[1]
                attr_fqn_name_in_param_dict = line.split("getattr(self.")[1].split("(")[0].replace(', "', '.').replace('")', '')
            else:
                # Example line in IR:
            ...
            attr_access_stmt = "tmp_value = " + line.split(" = ")[1].replace("self.", "locals()['fx_ir'].")
            exec(attr_access_stmt)
            attr_value = locals()['tmp_value']
            if isinstance(attr_value, torch.nn.Module):
                ...
                if attr_value in bufs_set:  # Buffer
                    ...
                    raise ValueError(
                        ...
                        "but this assumption no longer holds true for .{attr_fqn_name_in_param_dict}"
                    )
                    # line = line.replace(f"{attr_fqn_name_in_original_ir}", f"consts['{attr_fqn_name_in_param_dict}']")
            else:  # Const
                raise ValueError(f"non-module / non-tensor attribute is not supported, but found type of .{attr_fqn_name_in_param_dict} to be {type(attr_value)}")
                # line = line.replace(f"{attr_fqn_name_in_original_ir}", f"consts['{attr_fqn_name_in_param_dict}']")
...
def functionalize(module: torch.nn.Module, inputs, batch_dim, num_micro_batches):
    ...
    if alpa.torch.mode() == "dist":
        ...
        def convert_pt_module_to_jax_func(module, inputs):
            ...
            print(dict(_named_parameters(fx_ir)))
