...
SKIP_INPLACE = {
    v
    for v in itertools.chain(math.__dict__.values(), builtins.__dict__.values(), operator.__dict__.values())
    if callable(v)
}
...
def long_name(gm, node: torch.fx.Node):
    ...
    target = node.target
    if node.op == "call_function":
        return torch_get_name(node.target, f"{getattr(target, '__module__', '')}.{name}")
    elif node.op == "call_method":
        return name
    elif node.op == "call_module":
        ...
        for node in reversed(list(self.gm.graph.nodes)):
            ...
            if node.op == "call_function" and len(node.args) + len(kwargs) == 1:
                ...
                if isinstance(arg, torch.fx.Node) and counts[arg].usages == 0:
                    ...
                    elif " out: torch.Tensor" in repr(get_signature_for_torch_op(node.target)):
                        kwargs["out"] = arg
                        counters["optimizations"]["out"] += 1
                    else:
                        continue
                    with self.gm.graph.inserting_before(node):
                        node.replace_all_uses_with(self.gm.graph.call_function(node.target, node.args, kwargs))
...
class Functionalization(Transformer):
    ...
    def run_node(self, n: torch.fx.Node):
        ...
        for patch in patches:
            assert isinstance(patch, torch.fx.Node), f"{patch} {n.target} {n.args} {n.kwargs}"
    ...
    for node in list(graph.nodes):
        with graph.inserting_before(node):
            if node.op == "call_method" and node.target in NORMALIZE_METHODS:
                swap_node(
                    graph, node, graph.call_function(NORMALIZE_METHODS[node.target], node.args, node.kwargs),
                )
            elif node.op == "call_module":
                submod = gm.get_submodule(node.target)
                if submod.__class__.__name__ not in DONT_EXPAND_MODULES:
                    swap_node(
                        graph, node, expand_module_call(f"{node.target}.", graph, submod, node.args, node.kwargs),
                    )
