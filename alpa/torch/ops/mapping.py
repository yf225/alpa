...
def torch_conv2d(x, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    conv_out = lax.conv_general_dilated(
        ...
        dimension_numbers=lax.conv_dimension_numbers(
            x.shape, weight.shape, ("NCHW", "OIHW", "NCHW"),  # TODO: parameterize this! don't assume NCHW format.
        ),
    ...
    if bias is not None:
        ...
        bias_reshaped = jnp.broadcast_to(
            bias_reshaped, [conv_out.shape[0], bias.shape[0], conv_out.shape[2], conv_out.shape[3]]
        )
...
def torch_batch_norm(
    ...
    def _compute_stats(x, axes, axis_name: Optional[str] = None, axis_index_groups: Any = None):
        ...
        if axis_name is not None:
            ...
            mean, mean2 = jnp.split(
                lax.pmean(concatenated_mean, axis_name=axis_name, axis_index_groups=axis_index_groups), 2
            )
...
def torch_nn_init_xavier_uniform(x, gain: float = 1.0):
    ...
    return array_init_like(x, gen_uniform, minval=-a, maxval=a,)
...
def torch_nn_init_normal(x, mean: float = 0.0, std: float = 1.0):
    ...
    return array_init_like(x, gen_normal,)
